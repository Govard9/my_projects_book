...
class Aplication(Frame):
    ...
    def create_widgets(self):
        ...
        Checkbutton(self, text="Коммедия", variable=self.likes_comedy, command = self.update_text).grid(row=2,
                                                                                                        column=0,
                                                                                                        sticky=W)
        # флажок "Драма"
        self.likes_drama = BooleanVar()
        Checkbutton(self, text="Драма", variable=self.likes_drama, command=self.update_text).grid(row=3,
                                                                                                      column=0,
                                                                                                      sticky=W)
        # флажок "Фильм о любви"
        self.likes_romance = BooleanVar()
        Checkbutton(self, text="Фильм о любви", variable=self.likes_romance, command=self.update_text).grid(row=4,
                                                                                                      column=0,
                                                                                                      sticky=W)
